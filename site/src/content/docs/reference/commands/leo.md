---
title: "leo"
description: "由 leo CLI 命令树生成的命令参考。"
---

> 本页由 Cobra 命令树生成；命令、参数和输出保持 CLI 原文。

## leo

Personal command-line tools

```
leo [flags]
```

### Options

```
  -h, --help      help for leo
  -v, --version   Print version
```

### SEE ALSO

* [leo completion](./leo_completion.md)	 - Generate the autocompletion script for the specified shell
* [leo docker](./leo_docker.md)	 - Docker registry helpers
* [leo join](./leo_join.md)	 - Build a SQL IN value list from file or clipboard
* [leo log](./leo_log.md)	 - Open a temporary browser workspace for project logs
* [leo repo](./leo_repo.md)	 - Browse indexed repositories
* [leo shell](./leo_shell.md)	 - Shell integration helpers
* [leo time](./leo_time.md)	 - Convert timestamps and common time strings
* [leo version](./leo_version.md)	 - Print version

